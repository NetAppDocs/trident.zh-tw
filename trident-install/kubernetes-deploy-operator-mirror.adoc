...
在 `/deploy/operator.yaml`， 更新 `image: docker.io/netapp/trident-operator:25.10.0`反映您的影像註冊位置。你的link:../trident-get-started/requirements.html#container-images-and-corresponding-kubernetes-versions["Trident和csi影像"]CSI 映像檔可以位於同一個登錄檔或不同的登錄檔中，但所有 CSI 映像必須位於同一個登錄檔中。例如：

* `image: <your-registry>/trident-operator:25.10.0`如果您的所有圖片都位於同一個註冊表中。
* `image: <your-registry>/netapp/trident-operator:25.10.0`如果您的Trident鏡像與 CSI 映像位於不同的登錄檔。
