...
| `enableForceDetach` | 僅限`ontap-san`、 `ontap-san-economy`和 `ontap-nas-economy`。與 Kubernetes Non-Graceful Node Shutdown （ NGNS ）一起運作、讓叢集管理員能夠在節點發生問題時、將已掛載磁碟區的工作負載安全移轉至新節點。 | 「假」 
...
「強制分離」僅適用於 `ontap-san`、 `ontap-san-economy`和 `onatp-nas-economy`。啟用強制分離之前、必須先在 Kubernetes 叢集上啟用非正常節點關機（ NGNS ）。如需詳細資訊、請 link:https://kubernetes.io/docs/concepts/architecture/nodes/#non-graceful-node-shutdown["Kubernetes ：非正常節點關機"^]參閱。
...
[%collapsible]
====
本範例安裝 Astra Trident 、以搭配 GKE 叢集上的雲端身分識別使用。
