...
.步驟
. 如果您 link:../trident-get-started/kubernetes-deploy-helm.html#deploy-the-trident-operator-and-install-astra-trident-using-helm["使用 Helm 安裝 Astra Trident"]、您可以使用 `helm upgrade trident netapp-trident/trident-operator --version 24.02.0` 只需一步即可升級。如果您未新增 Helm repo 或無法使用它來升級：
+
