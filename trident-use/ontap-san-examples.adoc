...
[role="lead"]
在 Trident 安裝中建立和使用 ONTAP SAN 驅動程式。本節提供後端組態範例，以及將後端映射至 StorageClasses 的詳細資訊。link:https://docs.netapp.com/us-en/asa-r2/get-started/learn-about.html["ASA r2 系統"^]與其他 ONTAP 系統（ASA、AFF 和 FAS）在儲存層的實作上有所不同。這些差異會影響某些參數的使用，如註釋所示。link:https://docs.netapp.com/us-en/asa-r2/learn-more/hardware-comparison.html["詳細了解 ASA r2 系統與其他 ONTAP 系統之間的差異"^]。在 Trident 後端組態中，您無需指定系統為 ASA r2。當您選擇 `ontap-san`作為 `storageDriverName`時，Trident 會自動偵測 ASA r2 或其他 ONTAP 系統。如下表所示，某些後端組態參數不適用於 ASA r2 系統。
...
從 26.06 版本開始，您可以啟用 ONTAP 均衡放置，以便 ONTAP 為新 Volume 選擇彙總。請參閱 link:ontap-balanced-placement.html["使用 ONTAP 後端進行均衡放置"] 以瞭解需求和範例。
...
| 「torageDriverName」 | 儲存設備驅動程式的名稱。建立後無法修改。若要更新此參數，必須建立新的後端。 | `ontap-san`或 `ontap-san-economy` 
...
| 「toragePrefix」 | 在 SVM 中為新 Volume 進行資源配置時所使用的前置詞。之後無法修改。若要更新此參數，您需要建立新的後端。 | `trident` 
...
NOTE: 當 SVM 中的彙總更新時，Trident 會自動輪詢 SVM 並更新，無需重新啟動 Trident 控制器。如果您已在 Trident 中設定了特定的彙總來資源配置 Volume，並且該彙總被重新命名或從 SVM 移出，則在輪詢 SVM 彙總期間，Trident 中的後端將進入故障狀態。您必須將該彙總變更為 SVM 中存在的彙總，或將其完全移除，才能使後端恢復線上。
...
| `formatOptions`  a| 
使用 `formatOptions`為 `mkfs`命令指定命令列參數，此命令會在每次格式化 Volume 時套用。這可讓您根據自己的偏好格式化 Volume。請確保指定的 formatOptions 與 mkfs 命令選項類似，但不包含裝置路徑。例如："-E nodiscard"
...
| 加密 | 在新 Volume 上啟用 NetApp Volume Encryption (NVE)；預設為 `false`。必須在叢集上取得 NVE 授權並啟用，才能使用此選項。如果後端已啟用 NAE，則在 Trident 中資源配置的任何 Volume 都會啟用 NAE。如需詳細資訊，請參閱：link:../trident-reco/security-reco.html["Trident 如何與 NVE 和 NAE 搭配運作"]。 | “false” *如果指定，則設定為 `true`適用於 ASA r2 系統*。 
...
NOTE: 對於使用此 `ontap-san`驅動程式建立的所有 Volume，Trident 會額外增加 10% 的容量至 FlexVol，以容納 LUN 中繼資料。LUN 的資源配置大小與使用者在 PVC 中要求的大小完全一致。Trident 額外增加 10% 至 FlexVol（在 ONTAP 中顯示為可用大小）。使用者現在可獲得所要求的可用容量。此變更也可防止 LUN 在可用空間未完全使用之前變為唯讀狀態。此變更不適用於 ontap-san-economy。
...
以下 StorageClass 定義均參照 <<虛擬集區的後端範例>>。使用  `parameters.selector` 欄位，每個 StorageClass 都會指出哪些虛擬資源池可用於裝載 Volume。Volume 現在具有所選虛擬資源池中定義的屬性。

* The `protection-gold` StorageClass 映射到 `ontap-san` 後端的第一個虛擬資源池。這是唯一提供黃金級保護的資源池。
+
[source, yaml]
----
...
parameters:
  ...
  fsType: "ext4"
----
* The `protection-not-gold` StorageClass 映射至 `ontap-san`後端中的第二和第三個虛擬資源池。這些是唯一提供金級以外其他保護層級的資源池。
+
[source, yaml]
----
apiVersion: storage.k8s.io/v1
...
parameters:
  ...
  fsType: "ext4"
----
* The `app-mysqldb` StorageClass 映射到 `ontap-san-economy`後端中的第三個虛擬儲存資源池。這是唯一一個為 mysqldb 類型應用程式提供儲存資源池組態的儲存資源池。
+
[source, yaml]
----
apiVersion: storage.k8s.io/v1
...
parameters:
  ...
  fsType: "ext4"
----
* The `protection-silver-creditpoints-20k` StorageClass 對應至 `ontap-san` 後端中的第二個虛擬資源池。這是唯一提供白銀級保護和 20000 信用點的資源池。
+
[source, yaml]
----
apiVersion: storage.k8s.io/v1
...
parameters:
  ...
  fsType: "ext4"
----
* The `creditpoints-5k` StorageClass 分別對應 `ontap-san`後端中的第三個虛擬資源池和 `ontap-san-economy`後端中的第四個虛擬資源池。只有這兩個資源池產品提供 5000 creditpoints。
+
[source, yaml]
----
apiVersion: storage.k8s.io/v1
...
parameters:
  ...
  fsType: "ext4"
----
* StorageClass `my-test-app-sc`映射到 `testAPP`驅動程式中的虛擬資源池 `ontap-san`，並搭配 `sanType: nvme`。這是唯一提供 `testApp`的資源池。
+
[source, yaml]
----
---
