...
'''
sidebar: sidebar permalink: trident-use/ontap-nas-examples.html keywords: map backends, storage classes, trident backend, ontap-nas, ontap-nas-economy, ontap-nas-flexgroups, options, storageclasses, fsx, metrocluster summary: 了解如何使用 ONTAP NAS 驅動程式搭配您的 Trident 安裝。本節提供後端組態範例以及如何將後端對應到 StorageClasses 的詳細資訊。---
...
[role="lead"]
在 Trident 安裝中建立和使用 ONTAP NAS 驅動程式。本節提供後端組態範例，以及將後端映射至 StorageClasses 的詳細資訊。從 25.10 版本開始，NetApp Trident 支援link:https://docs.netapp.com/us-en/ontap-afx/index.html["NetApp AFX 儲存系統"^]。NetApp AFX 儲存系統與其他基於 ONTAP 的系統（ASA、AFF 和 FAS）在儲存層的實作上有所不同。
...
從 26.06 版本開始，您可以啟用 ONTAP 均衡放置功能，以便 ONTAP 為新 Volume 選擇彙總。ONTAP 9.19.1 或更新版本上的 AFX 儲存系統需要啟用均衡放置功能。有關需求和範例，請參閱 link:ontap-balanced-placement.html["使用 ONTAP 後端進行均衡放置"]。
...
NOTE: 當使用 ontap-nas-economy 且 storagePrefix 為 24 個或更多字元時，qtree 中不會嵌入儲存設備前綴，但它存在於 Volume 名稱中。
| " Trident " 


| 《Aggregate》  a| 
用於資源配置的彙總（可選；如果設定，則必須指派給 SVM）。對於 `ontap-nas-flexgroup`驅動程式，此選項將被忽略。如果未指派，則可以使用任何可用的彙總來資源配置 FlexGroup Volume。


NOTE: 當 SVM 中的彙總更新時，Trident 會自動輪詢 SVM 並更新，無需重新啟動 Trident 控制器。如果您已在 Trident 中設定了特定的彙總來資源配置 Volume，且該彙總被重新命名或從 SVM 移出，則在輪詢 SVM 彙總時，Trident 中的後端將進入故障狀態。您必須將該彙總變更為 SVM 中存在的彙總，或將其完全移除，才能使後端恢復線上。

*請勿指定用於 AFX 儲存系統。 *

*請勿指定何時 `useBalancedPlacement`設定為 `true`。*
 a| 
"
...
將新 Volume 的彙總選擇委託給 ONTAP [布林值]。需要 ONTAP 9.19.1 或更新版本，且 `useREST`設定為 `true`。不能與 `aggregate`組合使用。

請參考 link:ontap-balanced-placement.html["使用 ONTAP 後端進行均衡放置"] 以了解需求和範例。

*如果指定，對於 ONTAP 9.19.1 或更新版本的 AFF 儲存系統，一律設定為 `true`。*
| `false`; `true` 適用於 ONTAP 9.19.1 或更新版本的 AFX 儲存系統 
...
NOTE: 當 SVM 中的彙總列表更新時，Trident 會自動輪詢 SVM 來更新該列表，無需重新啟動 Trident 控制器。如果您已在 Trident 中設定了特定的彙總清單來資源配置 Volume，則如果該彙總清單已重新命名或從 SVM 中移出，Trident 在輪詢 SVM 彙總時會將後端置於故障狀態。您必須將該彙總列表變更為 SVM 中存在的列表，或將其完全移除，才能使後端恢復線上。
| " 
...
| 「nfsMountOptions」 | 以逗號分隔的 NFS 裝載選項清單。Kubernetes 持久性磁碟區的裝載選項通常在儲存設備類別中指定，但如果儲存設備類別中未指定任何裝載選項，Trident 將回退到使用儲存設備後端組態檔案中指定的裝載選項。如果儲存設備類別和組態檔案中均未指定裝載選項，Trident 將不會在關聯的持久性 Volume 上設定任何裝載選項。 | " 
...
| 加密 | 在新 Volume 上啟用 NetApp Volume Encryption (NVE)；預設為 `false`。必須在叢集上取得 NVE 授權並啟用，才能使用此選項。如果後端已啟用 NAE，則在 Trident 中資源配置的任何 Volume 都會啟用 NAE。如需詳細資訊，請參閱：link:../trident-reco/security-reco.html["Trident 如何與 NVE 和 NAE 搭配運作"]。 | "假" 
...
| 「napshotDir | 控制對的存取 `.snapshot` 目錄 | `true`,  `false`（明確設定）。 
...
升級 Trident 時，對於先前安裝的現有後端，請依照上述說明進行 Volume 資源配置。對於升級前建立的 Volume，您需要調整其大小才能使變更生效。例如，2 GiB 的 PVC 搭配 `snapshotReserve=50`先前產生的 Volume 僅提供 1 GiB 的可寫入空間。例如，將 Volume 調整為 3 GiB，即可為應用程式在 6 GiB 的 Volume 上提供 3 GiB 的可寫入空間。
...
[%collapsible]
====
此範例啟用 ONTAP 平衡放置，以便 ONTAP 為新 Volume 選擇彙總。平衡放置需要 ONTAP 9.19.1 或更新版本。如需詳細資訊，請參閱 link:ontap-balanced-placement.html["使用 ONTAP 後端進行均衡放置"]。
...
以下 StorageClass 定義均參照 <<虛擬集區的後端範例>>。使用  `parameters.selector` 欄位，每個 StorageClass 都會指出哪些虛擬資源池可用於裝載 Volume。Volume 具有所選虛擬資源池中定義的屬性。

* The `protection-gold` StorageClass 對應至 `ontap-nas-flexgroup`後端中的第一個和第二個虛擬資源池。這些是唯一提供黃金層級保護的資源池。
+
[source, yaml]
----
...
parameters:
  ...
  fsType: "ext4"
----
* The `protection-not-gold` StorageClass 對應至 `ontap-nas-flexgroup`後端中的第三和第四個虛擬資源池。這些是唯一提供黃金以外保護層級的資源池。
+
[source, yaml]
----
apiVersion: storage.k8s.io/v1
...
parameters:
  ...
  fsType: "ext4"
----
* The `app-mysqldb` StorageClass 映射到 `ontap-nas`後端中的第四個虛擬儲存資源池。這是唯一一個為 mysqldb 類型應用程式提供儲存資源池組態的儲存資源池。
+
[source, yaml]
----
apiVersion: storage.k8s.io/v1
...
parameters:
  ...
  fsType: "ext4"
----
* The `protection-silver-creditpoints-20k` StorageClass 對應至 `ontap-nas-flexgroup` 後端中的第三個虛擬資源池。這是唯一提供白銀級保護和 20000 信用點的資源池。
+
[source, yaml]
----
apiVersion: storage.k8s.io/v1
...
parameters:
  ...
  fsType: "ext4"
----
* The `creditpoints-5k` StorageClass 分別對應 `ontap-nas`後端中的第三個虛擬資源池和 `ontap-nas-economy`後端中的第二個虛擬資源池。只有這兩個資源池產品提供 5000 creditpoints。
+
[source, yaml]
----
apiVersion: storage.k8s.io/v1
