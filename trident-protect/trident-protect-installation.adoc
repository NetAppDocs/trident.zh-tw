...
NOTE: 若要取得 Helm chart，請先在可存取互聯網的機器上使用 `helm pull trident-protect --version 100.2602.0 --repo https://netapp.github.io/trident-protect-helm-chart`下載 Helm chart，然後將產生的 `trident-protect-100.2602.0.tgz`檔案複製到您的離線環境中，並在最後一步中使用 `helm install trident-protect ./trident-protect-100.2602.0.tgz`而不是儲存庫引用進行安裝。
