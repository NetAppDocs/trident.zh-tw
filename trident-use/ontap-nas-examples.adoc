...
[%collapsible%open]
====
這是使用的基本組態 `ontap-nas-economy` 驅動程式：
...
----
====
.ONTAP NAS FlexGroup 最小化組態範例
[%collapsible%open]
====
這是使用的基本組態 `ontap-nas-flexgroup` 驅動程式：
...
[%collapsible%open]
====
此範例指示 Astra Trident 使用動態匯出原則來自動建立及管理匯出原則。這對的運作方式相同 `ontap-nas-economy` 和 `ontap-nas-flexgroup` 驅動程式：
...
[%collapsible%open]
====
此範例使用 SMB 磁碟區為 ONTAP 後端建立 Amazon FSX 。
