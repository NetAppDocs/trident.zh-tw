...
| 「napshotDir | 存取「.snapshot」目錄 | `true`,  `false`（預設行為可能有所不同。請明確設定）NFSv3 設定為 "false" 
...
如果您剛開始使用 Google Cloud NetApp Volumes 並進行嘗試，此配置是理想的選擇，但在實踐中，您可能需要為配置的磁碟區提供額外的範圍。


NOTE: 請將 `<id_value>`和 `<key_value>`替換為您的服務帳戶憑證。
...
[%collapsible]
====
當您使用 GKE 雲端身分安裝 Trident 時，後端會以工作負載身分進行驗證，並省略 `apiKey`區塊。Google Cloud NetApp Volumes 驅動程式的雲端身分配置方式相同。唯一的差異在於 `storageDriverName`值：NAS（NFS 或 SMB）磁碟區使用 `google-cloud-netapp-volumes`，SAN（iSCSI 區塊）磁碟區使用 `google-cloud-netapp-volumes-san`。


NOTE: 驅動程式對雲端身分的支援需要 Trident 26.06 或更高版本。 `google-cloud-netapp-volumes-san`SAN 磁碟區是從 Flex Unified 儲存池中佈建的，因此請將 `serviceLevel`設定為 `flex`。

.NAS 驅動程式（<code>google-cloud-netapp-volumes</code>）
[source, yaml]
----
apiVersion: trident.netapp.io/v1
kind: TridentBackendConfig
...
----
.SAN 驅動程式（<code>google-cloud-netapp-volumes-san</code>）
[source, yaml]
----
apiVersion: trident.netapp.io/v1
kind: TridentBackendConfig
