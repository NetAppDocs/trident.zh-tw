...
* * Kubernetes* ：
+
** 修正了 CSI 發布和 Volume 移動之間的競爭條件，該條件可能導致使用過時的存取資訊附加 Volume。
** 修復了 NodeGetInfo 在節點註冊完成之前可能返回空拓撲標籤的競爭條件，進而導致拓撲感知佈建失敗。
** 修復了一個罕見的 NVMe 斷開連接競爭問題，該問題可能在另一個 pod 仍在連接時拆除共享的子系統。
** 修正了在並發條件下建立掛載 Volume 的 Clone 時，NVMe PVC Clone 發布失敗（以及父命名空間映射錯誤）的問題。
** 修正了使用 nameTemplate 的 ONTAP-SAN-Economy 後端上 PVC 快照建立失敗的問題。
...
* * Kubernetes* ：
+
** 在 ONTAP 後端設定中新增選用的 `storageAPITimeout`（秒），用於設定每個後端的 REST 和 ZAPI API 用戶端逾時時間。
...
* * Kubernetes* ：
+
** 為 ONTAP-NAS-Economy 和 ONTAP-SAN-Economy 驅動程式新增並行處理的正式版 （GA） 支援，使此功能從技術預覽版升級，並為 Azure NetApp Files （ANF） 和 SolidFire 驅動程式新增 GA 並行處理支援。
** 新增對 Kubernetes 1.36 的支援。
** 為 Google Cloud NetApp Volumes (GCNV) NAS 和 SAN 驅動程式新增了磁碟區自動成長的正式版 (GA) 支援。
** 新增統一的 ONTAP 平衡放置功能，以便在佈建新磁碟區時實現最佳集合體選擇（link:https://github.com/NetApp/trident/issues/64["問題 #64"]）。
** 為 ONTAP-NAS-Flexgroup 驅動程式新增了 ONTAP AFX 支援。
** 新增與 NetApp Shift Toolkit 的整合，以在使用 Red Hat OpenShift Migration Toolkit for Virtualization 時簡化虛擬機器移轉。
** 為 GCNV SAN 驅動程式新增了 Cloud Identity 支援。
** 已為 Trident Volume Publication （TVP） 傳播和自動增長工作流程新增從屬磁碟區支援。
** 新增在啟用並行功能的情況下執行 Trident 時，對磁碟區更新和磁碟區群組快照的支援。
** 為 GCNV 新增動態 gRPC 速率限制。
** 新增了控制器就緒執行探測，並調整了控制器和邊車的預設 Kubernetes API QPS 和 Burst 設定。
** 改進節點操作順序，將附加、加密設定和檔案系統格式化步驟解耦。
** 將 Trident 容器根檔案系統掛載為唯讀，以提高安全性。
** 在 ONTAP 後端中，當複製磁碟區的來源快照被刪除時，會盡力刪除複製磁碟區快照。
** 新增備份複製 PVC 識別，以防止由 Veeam Kasten、Trilio 和 Cohesity 建立的臨時 PVC 卡在 ONTAP 磁碟區復原佇列中。
** 增強了 AWS ARN 處理能力，以支援特殊區域。
** 防止 ONTAP-SAN-ECONOMY 驅動程式在調整大小操作期間 Flexvols 縮小。
...
* *[Tech Preview]*：新增對 ONTAP-NAS-Economy qtree 匯入的支援。
* *[Tech Preview]*：為 ASA-R2 （SAN 和 NVMe）驅動程式新增並行支援。
* *[Tech Preview]*：為 ONTAP-SAN 和 ONTAP-NAS 驅動程式新增了 GCNV ONTAP 模式 HTTP 傳輸。
...
* * Kubernetes* ：
+
** 修復了並行發布/取消發布、複製、快取和後端更新作業的競爭條件和狀態處理。
** 修正了 ONTAP-NAS 和 ONTAP-NAS-Economy 驅動程式中從屬磁碟區和唯讀複製的匯出原則競爭條件及並行發布/取消發布處理問題。
** 已修正當快照原則不同時，在 MetroCluster 容錯移轉後的 ONTAP-NAS-Economy FlexVol 探索 （link:https://github.com/NetApp/trident/issues/1082["問題 #1082"]）。
** 修正了 ONTAP-NAS-Economy 磁碟區刪除重試中斷長時間執行的 qtree 刪除的問題（link:https://github.com/NetApp/trident/issues/1121["問題 #1121"]）。
** 修正了 ONTAP-SAN 和 ONTAP-SAN-Economy 的匯入和調整大小行為，包括 fsType 驗證、磁碟區元資料處理和自動成長模式行為。
** 修正了並行建立磁碟區期間的 NVMe/TCP 命名空間競爭條件（link:https://github.com/NetApp/trident/issues/1089["問題 #1089"]）。
** 修正了當可用路徑和入口網站不符時發生的 iSCSI 調整大小失敗問題，並改善了重複調整大小的行為。
** 修正了由磁碟區繁忙狀態引起的 ONTAP-SAN 磁碟區刪除失敗問題。
** 修正了 ONTAP REST 錯誤處理問題，並改進了 SVM 狀態逾時行為。
** 修正了 GCNV SAN 區域集區和主機群組處理、標準化磁碟區識別碼轉換，以及標準化低於最小大小的建立要求。
** 修正了原生 GCNV 容量池選擇問題，以排除 ONTAP 模式集區。
** 修正了使用 Trident 自動工作負載故障轉移的虛擬機器的 KubeVirt 故障轉移問題，以加快虛擬機器故障轉移速度。
** 修正了 Operator 管理的安裝中 RBAC 問題，包括節點缺失 ClusterRoles 以及升級期間 ClusterRole 和 ClusterRoleBinding 的錯誤協調問題。此修復適用於使用 Trident Operator chart 的 Operator 和 Helm 型安裝。
** 修正了 GitOps 在 TridentConfigurator 和相關 CRD 上的持久差異，方法是省略預設優先值並修正 YAML 範本縮排（link:https://github.com/NetApp/trident/issues/915["問題 #915"]、link:https://github.com/NetApp/trident/issues/1120["問題 #1120"]）。
** 修正了在 Helm 升級期間修補 TridentVolumePublication CRD 的問題（link:https://github.com/NetApp/trident/issues/1151["問題 #1151"]）。
** 透過在節點註冊重試之前啟動 CSI gRPC 服務來修復 CSI 啟動順序問題。
** 修正了並行作業期間安全 SMB 複製組態覆蓋的問題。
** 修正了升級後 Trident 控制器 pod 上缺少的註解（link:https://github.com/NetApp/trident/issues/1004["問題 #1004"]）。
** 改進大型 SVM 上的慢速 qtree 配置（link:https://github.com/NetApp/trident/issues/1142["問題 #1142"]）。
** 已修復 ControllerPublish，現在使用磁碟區組態檔案系統類型，而不是錯誤的預設檔案系統類型。
** 修復了孤立的 dm 設備可能導致 CSI 重複 NodeUnstageVolume 故障的問題。
** 改進了過時節點和多路徑清理，在軟刪除節點清理後移除幽靈多路徑成品和過時的 TridentNode 資源。
...
* 新增了 OpenShift Service CA Operator 整合，用於自動化 Trident Protect 准入 Webhook 的 TLS 憑證管理。可透過在 Helm 值中將 `useOpenshiftServiceCA`設定為 `true`來啟用。若要深入瞭解，請參閱link:trident-protect/trident-protect-customize-installation.html["自訂Trident Protect 安裝"^]。
* 新增在 OpenShift Virtualization 上備份與還原 KubeVirt VM 的支援。保護命名空間中的所有 VM，或依名稱指定特定的 VM，並從備份或快照中選擇性地還原個別 VM。若要深入瞭解，請參閱 link:trident-protect/trident-protect-kubevirt-vms.html["使用 Trident Protect 保護 KubeVirt 虛擬機器"^]。
* 新增了對資源轉換的支援，讓您在還原過程中新增、刪除和修改 Kubernetes 資源屬性。若要深入瞭解，請參閱 link:trident-protect/trident-protect-resource-transformations.html["使用自訂資源還原資源時修改資源"]。
* 新增了對 AppMirrorRelationship （AMR）中 `storageClassMapping`的支援，可在單一 AMR 定義中為各個 PVC 進行儲存類別對應。如需深入瞭解，請參閱link:trident-protect/trident-protect-use-snapmirror-replication.html["使用NetApp SnapMirror和Trident Protect 複製應用程式"^]。
...
* 修正了當來源應用程式命名空間在目標叢集上不存在時，跨叢集虛擬機器還原失敗並出現准入 webhook 錯誤的問題。
* 修正了由於與現有應用程式自訂資源的規格不符，導致選擇性虛擬機器就地還原在應用程式建立步驟失敗的問題；現在可重複使用現有應用程式。
* 修正了虛擬機器就地復原作業在虛擬機器及其 PersistentVolumeClaim 均被刪除時停滯於 `Running`狀態的問題；缺少 PersistentVolumeClaim 現在將被視為嚴重錯誤。
* 修正了編輯現有保護排程時錯誤地觸發立即備份或快照執行的問題。
* 修正了快照終結器在應用程式自訂資源被刪除後嘗試獲取該資源的問題；現在，如果找不到應用程式，終結器會跳過此步驟。
* 修正在 PersistentVolumeClaim 複製步驟中使用稀疏複製進行虛擬機器就地還原後，固定保護儲存容量大幅增加的問題。
* 修正了 Trident Protect 在 ONTAP 中確認刪除後端快照之前刪除 Kubernetes 快照物件的問題；現在終結器會等待 `VolumeSnapshot`刪除。
* 修正了虛擬機器凍結操作失敗時快照無法繼續執行的問題；現在凍結和解凍操作在失敗時仍會繼續進行。
* 已修正 KubeVirt 虛擬機器應用程式在 AppMirrorRelationship 反向、反向重新同步和重新同步作業期間，錯誤地重新建立 PersistentVolumeClaims 的問題。
...
* * Kubernetes* ：
* 已修正 Google Cloud NetApp Volumes（GCNV）SAN 行為，以支援區域性 Flex Unified ZONAL 資源池用於區塊（iSCSI）工作負載。現在同時支援區域性與區域間的組態。
* 修正 Google Cloud NetApp Volumes（GCNV）NAS UNIFIED 資源池的處理方式，透過改進磁碟區建立、驗證、命名以及在 `InternalID`不存在時的備援磁碟區查找操作。
* 修正了當可用路徑和 portal 不符時，iSCSI 裝置調整大小作業可能會失敗的問題。
* 修正了  `blkid` 錯誤識別 LUKS 裝置的檔案系統類型、導致無法建立新磁碟區的間歇性問題。
* 修正了大規模 LUKS 發布和暫存操作可能導致 Trident 控制器阻塞的問題。現在使用者需要追蹤與磁碟區關聯的 LUKS 密碼短語。
* 修正了 iSCSI 多路徑分割區可能顯示為幽靈裝置，導致 CSI `NodeUnstage` 操作無法成功的問題。
* 修正了並行建立磁碟區期間的 NVMe/TCP 命名空間競爭條件（Issue #1089）。
* 修正了在同一秒內執行快照時並發克隆快照操作的問題。
* 修正了阻止使用 `dataVolumes`的 KubeVirt VM 透過 Trident Automated Workload Failover 進行故障轉移的問題。
* 修正了在使用 Trident 並發核心時刪除無效 `TridentMirrorRelationship`自訂資源時發生的錯誤。
* 修正了  `tridentactionmirrorupdates` Kubernetes CRD YAML 定義中的縮排問題（問題 #1120）。
* 修正了 REST qtree 非同步刪除 API 呼叫（Issue #1121）。
* 已修復 `ControllerPublish`，現在使用磁碟區組態檔案系統類型，而不是錯誤的預設檔案系統類型。
* 修正了控制器啟動日誌記錄，使其能夠正確報告並行驅動程式狀態。
* 修正了並發 Trident 中對已暫停後端的處理。
* 增強了 AWS ARN 處理能力，以支援特殊區域。
...
更新了 Trident Protect 依賴項以解決安全漏洞：
...
* Go 執行階段：link:https://nvd.nist.gov/vuln/detail/CVE-2025-22871["CVE-2025-22871"^]
* 其他相依性：link:https://nvd.nist.gov/vuln/detail/CVE-2025-23109["CVE-2025-23109"^]
...
* * Kubernetes* ：
+
** 新增 ONTAP-NAS（僅限 NFS）、ONTAP-SAN（iSCSI、FCP、NVMe）和 Google Cloud NetApp Volumes（GCNV）驅動程式並發支援的正式版（GA），此功能已從技術預覽版升級。如需詳細資訊，請參閱 link:https://docs.netapp.com/us-en/trident/trident-use/controller-scalability.html["控制器擴充性文件"]。
** 新增了對 Trident 基於自訂使用者定義 Trident AutoGrow 策略的磁碟區自動成長支援。如需詳細資訊，請參閱 link:https://docs.netapp.com/us-en/trident/trident-use/automatic-volume-expansion.html["自動磁碟區擴充文件"]。
** 增強了 Trident 節點的並發性，以提高 NVMe 磁碟區節點操作的可擴展性。如需詳細資訊，請參閱 link:https://docs.netapp.com/us-en/trident/trident-use/trident-nvme-rwx-subsystem-limits.html["NVMe Volume 說明文件"]。
** 新增對 Google Cloud NetApp Volumes NAS 磁碟區自動分層的支援（透過 PVC 註解 (`tieringPolicy`、 `tieringMinimumCoolingDays`），包括池選擇和克隆繼承。如需詳細資訊，請參閱 link:https://docs.netapp.com/us-en/trident/trident-use/gcnv-auto-tiering.html["設定 Google Cloud NetApp Volumes 文件的自動分層"]。
** 新增對使用  `google-cloud-netapp-volumes-san` 驅動程式的 GCNV SAN 區塊（iSCSI）磁碟區的支援，包括資源配置、用於 LUN 存取的每節點主機群組對應，以及從磁碟區複製作業。如需詳細資訊，請參閱 link:https://docs.netapp.com/us-en/trident/trident-use/gcnv-block.html["Google Cloud NetApp Volumes 區塊組態文件"]。
** 新增對 Amazon FSx for NetApp ONTAP 自動後端組態的支援。當您建立包含所需參數的 StorageClass 時、 Trident 會自動建立對應的後端和 VolumeSnapshotClass （如有需要）。如需詳細資訊、請參閱 link:https://docs.netapp.com/us-en/trident/trident-use/trident-fsx-auto-backend-configuration.html#add-additional-fsxn-file-systems["Amazon FSx for NetApp ONTAP 後端組態文件"] 。
** 增加了對不同 Microsoft Azure 雲端的支援，例如 Azure Government 和 Azure China，以及 Azure NetApp Files 後端的自訂雲端組態。如需詳細資訊，請參閱 link:https://docs.netapp.com/us-en/trident/trident-use/anf.html["Azure NetApp Files 後端組態文件"]。
** 新增對 Kubernetes 1.35 的支援。如需詳細資訊，請參閱 link:https://docs.netapp.com/us-en/trident/trident-get-started/requirements.html["需求文件"]。
...
* * Kubernetes* ：
+
** 修正了在 ONTAP-NAS、ONTAP-NAS-Economy 和 Google Cloud NetApp Volumes 驅動程式中，取消發布只讀複製會從來源磁碟區中刪除匯出策略規則的問題link:https://github.com/NetApp/trident/issues/1086["問題 #1086"]。
** 將 kubectl 映像切換為基於 Alpine 的輕量級變體，以防止 Bitnami 公開映像棄用後出現拉取失敗 https://github.com/NetApp/trident/issues/1080["問題 #1080"]。
** 修正了在 Trident 升級期間保留現有部署註解的問題link:https://github.com/NetApp/trident/issues/1004["問題 #1004"]。
** 如果兩個儲存類別都指向同一個後端 link:https://github.com/NetApp/trident/issues/1104["問題 #1104"]，則允許跨不同儲存類別進行複製。
...
** 改進了 CSI 和 ONTAP 克隆分割逾時，以避免與某些備份應用程式出現競爭條件 link:https://github.com/NetApp/trident/issues/1098["問題 #1098"] link:https://github.com/NetApp/trident/issues/1100["問題 #1100"]。
** 修正了 LUKS 錯誤訊息的抑制問題link:https://github.com/NetApp/trident/issues/1069["問題 #1069"]。
** 修正了 iSCSI 和 NVMe 協定中過期 LUKS 映射器的處理問題。增強的清理邏輯可防止因孤立的裝置映射器而導致的掛載失敗。


* 修正了 RWX NVMe Volume 的規模限制。
* 更新了 OpenTelemetry-Go 套件以修復 link:https://nvd.nist.gov/vuln/detail/cve-2026-24051["CVE-2026-24051"]。
...
* Trident Protect 現在會在執行就地還原之前自動停用保護排程並取消正在進行的作業，並在還原完成後重新啟用它們。如需深入瞭解，請參閱 link:trident-protect/trident-protect-restore-apps.html["使用Trident Protect 恢復應用程式"^]。
* 已將 `runImmediately`欄位新增至排程 CR 和 `--run-immediately` CLI 標誌中，以便在建立排程時立即觸發備份或快照。如需深入瞭解，請參閱link:trident-protect/trident-protect-protect-apps.html#create-a-data-protection-schedule["建立資料保護排程"^]。
* 新增使用 restore CR 中的 `destinationApplicationName`欄位或 `--destination-app-name` CLI 旗標為還原的應用程式指定自訂名稱的支援。如需深入瞭解、請參閱link:trident-protect/trident-protect-restore-apps.html["使用Trident Protect 恢復應用程式"^]。
...
+
看link:https://docs.netapp.com/us-en/trident/trident-protect/trident-protect-protect-apps.html["支援的備份和計劃註釋"^]。



* 在計劃 CR 中新增了一個註釋，用於配置 PVC 綁定逾時，該逾時將由備份 CR 使用： `protect.trident.netapp.io/pvc-bind-timeout-sec` 。看link:https://docs.netapp.com/us-en/trident/trident-protect/trident-protect-protect-apps.html["支援的備份和計劃註釋"^]。
...
IMPORTANT: 對於正在使用 SolidFire 的客戶，請勿升級至 25.06.1，因為在取消發佈磁碟區時存在已知問題。25.06.2 即將發佈以解決此問題。
...
** 修正了 GCNV 磁碟區在 `nfsMountOptions`未設定時預設使用 NFSv3 的問題；現在同時支援 NFSv3 和 NFSv4 協定。如果未提供 `nfsMountOptions`，則使用主機的預設 NFS 版本（NFSv3 或 NFSv4）。
