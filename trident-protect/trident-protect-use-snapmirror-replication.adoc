...
NOTE: 使用 CR 時，請在套用 CR 之前手動建立目標命名空間。Trident Protect 僅在使用 CLI 時才會自動建立命名空間。
...
*** *spec.storageClassMapping*: （_可選_）將同一 AMR 內各個 PVC 的來源儲存類別對應到目標儲存類別。每個條目都需要一個 `source`和 `destination`儲存類別名稱。設定後， `storageClassMapping`將完全覆蓋 `storageClassName`；沒有符合來源條目的 PVC 將使用叢集預設儲存類別，而非 `storageClassName`。
+
在 AppMirrorRelationship CR 中使用 `storageClassMapping`時，請注意以下限制：

+
**** `storageClassMapping` 覆蓋  `storageClassName` 的全部內容。當兩個欄位都設定時， `storageClassName` 將被忽略。
**** 來源儲存類別在 `storageClassMapping`中沒有相符條目的 PVC 將使用叢集預設儲存類別，而非 `storageClassName`。
**** 每個條目都需要一個 `source`和 `destination`值。
**** 更改目標儲存類別可能會強制刪除並重新建立現有的目標 PVC。
...
NOTE: 使用 `--storageclass-mapping`將各個來源儲存類別對應到目標儲存類別。指定一個或多個 `source:destination`以逗號分隔的鍵值對（例如， `"class-a:class-b,class-c:class-d"`）。使用 `*:destination-class`作為萬用字元，可將所有未對應的 PVC 對應到單一目標儲存類別。當 `--storageclass-mapping`設定後， `--storage-class`將被忽略。

+
*使用單一目標儲存類別的範例 (`--storage-class`）：*
