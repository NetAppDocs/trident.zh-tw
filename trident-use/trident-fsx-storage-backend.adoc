...
本節中的步驟涵蓋如何單獨建立後端。有關如何使用自動後端功能的資訊，請參閱xref:trident-fsx-storageclass-pvc.html#configure-a-kubernetes-storageclass-object-with-automatic-backend-configuration-support-for-fsx-for-ontap[配置 StorageClass 和 PVC]章節以取得更多資訊。
...
您可以使用以下方式設定 SMB 磁碟區： `ontap-nas`司機。在你完成之前 <<ONTAP SAN and NAS driver integration>> 請完成以下步驟： link:https://docs.netapp.com/us-en/trident/trident-use/worker-node-prep.html#prepare-to-provision-smb-volumes["準備配置SMB磁碟區"]。
