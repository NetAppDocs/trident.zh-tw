...
本節中的步驟涵蓋如何單獨建立後端。有關如何使用自動後端功能的資訊，請參閱link:trident-fsx-storageclass-pvc.html#configure-a-kubernetes-storageclass-object-with-automatic-backend-configuration-support-for-fsx-for-ontap["配置 StorageClass 和 PVC"]章節以取得更多資訊。
...
您可以使用  `ontap-nas` 驅動程式來配置 SMB 磁碟區。在完成 link:trident-fsx-storage-backend.html#ontap-san-and-nas-driver-integration["ONTAP SAN 和 NAS 驅動程式整合"] 之前，請先完成以下步驟：link:worker-node-prep.html#prepare-to-provision-smb-volumes.html["準備配置SMB磁碟區"]
