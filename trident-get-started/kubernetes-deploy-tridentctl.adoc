...
[role="lead"]
您可以使用安裝Astra Trident `tridentctl`。此程序適用於Trident和csi映像位於的安裝 `docker.io` 和 `registry.k8s.io` 或是私有的鏡射登錄。以自訂您的 `tridentctl` 部署、請參閱 link:kubernetes-customize-deploy-tridentctl.html["自訂試用部署"]。
