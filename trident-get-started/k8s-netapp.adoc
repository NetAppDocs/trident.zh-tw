...
https://www.netapp.com/google-cloud/netapp-volumes/["Google Cloud NetApp Volumes"^] 是一項雲端原生檔案服務，可透過 NFS 提供全快閃效能的 NAS 磁碟區。此服務支援包括傳統應用程式在內的任何工作負載在 GCP 雲端上執行。它提供完全託管的服務，可實現一致的高效能、即時複製、資料保護，以及對 Google Compute Engine (GCE) 執行個體的安全存取。
