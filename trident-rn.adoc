...
* * Kubernetes* ：
+
** 增加了對 CSI 卷組快照的支持 `v1beta1`適用於 ONTAP-SAN iSCSI 驅動程式的磁碟區組快照 Kubernetes API。請參閱。 link:../trident-use/vol-group-snapshots.html["使用磁碟區組快照"^]
** 除了 iSCSI 之外，還增加了對 NVMe/TCP 的 ONTAP ASA r2 的支援。請參閱。 link:../trident-use/ontap-san-examples.html["SAN組態選項與範例ONTAP"^]
** 為 ONTAP-NAS 和 ONTAP-NAS-Economy 磁碟區新增了安全的 SMB 支援。 ActiveDirectory 使用者和群組現在可以與 SMB 磁碟區一起使用，以增強安全性。請參閱。 link:../trident-use/ontap-nas-prep.html#enable-secure-smb["啟用安全 SMB"^]
