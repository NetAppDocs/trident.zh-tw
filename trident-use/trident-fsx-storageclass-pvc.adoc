...
您可以透過 StorageClass 傳遞可選的後端參數。在 StorageClass `parameters` 部分中將所有可選值定義為字串。有關後端參數的完整列表、請參閱：xref:trident-fsx-storage-backend.html#backend-advanced-configuration-and-examples[FSx for NetApp ONTAP 後端組態]。
...
您可以使用  `ontap-nas` 驅動程式來配置 SMB 磁碟區。但是，若要執行此操作，您必須完成以下步驟：link:https://docs.netapp.com/us-en/trident/trident-use/worker-node-prep.html#prepare-to-provision-smb-volumes["準備配置SMB磁碟區"]。
