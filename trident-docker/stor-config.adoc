...
*《SAN驅動程式*：請勿指定用於iSCSI》ONTAP 。Trident 使用link:https://docs.netapp.com/us-en/ontap/san-admin/selective-lun-map-concept.html["可選擇的LUN對應ONTAP"^]來探索建立多重路徑工作階段所需的 iSCI 生命。如果明確定義、就會產生警告 `dataLIF`。
 a| 
`10.0.0.2`
...
`300`
 ...
 a| 
`sanType`
| * 僅支援 `ontap-san`驅動程式。 *用於選擇 `iscsi` iSCSI 、 `nvme` NVMe / TCP 或 `fcp` SCSI over Fibre Channel （ FC ）。*'FCP （ SCSI over FC ）是 Trident 24.10 版本的技術預覽功能。 * | `iscsi` 如果空白 
