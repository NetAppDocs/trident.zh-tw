...
[role="lead"]
了解使用 ONTAP NAS 驅動程式配置 ONTAP 後端的要求、驗證選項和匯出原則。從 25.10 版本開始，NetApp Trident 支援 link:https://docs.netapp.com/us-en/ontap-afx/index.html["NetApp AFX儲存系統"^]。NetApp AFX 儲存系統與其他 ONTAP 系統（ASA、AFF 和 FAS）在儲存層的實作方式上有所不同。在 Trident 後端組態中、您無需指定系統為 AFX。當您選擇 `ontap-nas`作為 `storageDriverName`時、Trident 會自動偵測 AFX 系統。
