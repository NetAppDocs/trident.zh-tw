...
在使用 Trident 配置磁碟區之前，您需要透過建立 Trident Backend Configuration（TBC）將儲存後端與 Trident 整合。TBC 定義了 Trident 如何連接到儲存後端，並提供 Trident 管理儲存資源所需的必要資訊。



[[create-a-trident-backend-configuration-tbc-for-fsx-for-ontap]]
== 為 FSx for ONTAP 建立 Trident 後端組態（TBC）

有兩種方法可以設定 Trident 後端以使用 FSx for ONTAP 檔案系統：* 建立一個利用自動後端功能的 StorageClass。* 分別建立後端和 StorageClass。

本節中的步驟涵蓋如何單獨建立後端。有關如何使用自動後端功能的資訊，請參閱link:trident-fsx-storageclass-pvc.html#configure-a-storage-class-without-automatic-backend-configuration["配置 StorageClass 和 PVC"]章節以取得更多資訊。
...
| 「torageDriverName」 | 儲存設備驅動程式的名稱。建立後無法修改。若要更新此參數，必須建立新的後端。 | `ontap-nas`、 `ontap-nas-economy`、 `ontap-nas-flexgroup`、 `ontap-san`、 `ontap-san-economy` 
...
| 「toragePrefix」 | 在SVM中配置新磁碟區時所使用的前置碼。無法在建立後修改。若要更新此參數、您需要建立新的後端。 | `trident` 注意：影響後端識別和 Volume 命名的後端組態參數（例如  `storageDriverName` 和  `storagePrefix`）在建立後無法變更。若要更新它們，您必須建立新的後端。 
...
您可以使用  `ontap-nas` 驅動程式來配置 SMB 磁碟區。在完成 link:trident-fsx-storage-backend.html#fsx-for-ontap-driver-details["ONTAP SAN 和 NAS 驅動程式整合"] 之前，請先完成以下步驟：link:worker-node-prep.html#prepare-to-provision-smb-volumes["準備配置SMB磁碟區"]
