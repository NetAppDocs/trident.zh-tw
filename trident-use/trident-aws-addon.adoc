...
. 配置所需的附加元件設定。
+
image::../media/aws-eks-04.png[Trident 附加元件組態設定]

. 如果您使用 IRSA（服務帳戶的 IAM 角色），請參閱其他設定步驟link:https://docs.netapp.com/us-en/trident/trident-use/trident-fsx-install-trident.html#enable-the-trident-add-on-for-aws["請按這裡"]。
. 選擇* Create *（建立*）。
. 確認附加元件的狀態為 _Active_ 。
+
image::../media/aws-eks-05.png[Trident 附加元件啟動狀態]
...
----
. 繼續進行設定並配置儲存後端。如需相關資訊、請參閱 link:trident-fsx-storage-backend.html["設定儲存後端"]。
