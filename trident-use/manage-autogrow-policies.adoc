...
[role="lead"]
建立 Autogrow Policies 後，您可以檢視、更新和刪除它們。您還可以監控哪些磁碟區使用了特定的原則。
...
IMPORTANT: 變更會影響所有使用該策略的磁碟區。如果可能，請先在非生產環境中測試變更。
...
* *立即生效：*所有使用此原則的磁碟區將在下次成長評估時採用新參數。
* *無需重新啟動磁碟區：*變更將套用於下一次成長作業。
* *先測試：* 盡可能在非生產環境中驗證變更。
...
自動增長原則使用終結器保護，以防止磁碟區在積極使用時意外刪除。
...
----
. 如果磁碟區仍然使用該原則，則刪除作業將進入 `Deleting`狀態。請檢查哪些磁碟區受到影響：
+
[source, console]
----
tridentctl get autogrowpolicy production-db-policy -o yaml
...
| 磁碟區使用此原則 | 策略進入  `Deleting` 狀態。終結器會阻止完成，直到所有磁碟區都被移除。 
...
Autogrow 支援 ONTAP 後端。Autogrow 也支援 Google Cloud NetApp Volumes （GCNV） 的 NAS 和 SAN 磁碟區。
...
* *現有磁碟區（棕地部署）：*即使應用了有效的自動成長策略，自動成長功能也可能無法對現有磁碟區生效。此限制是由於磁碟區發布正在進行遷移所致。若要確認遷移已完成，請檢查 Trident 控制器日誌中的 `"Migration completed"`訊息。
...
PVC 註解始終優先。如果 PVC 具有 `trident.netapp.io/autogrowPolicy`註解，Trident 將使用該值，而不管 StorageClass 中指定了什麼值。詳情請參閱 link:automatic-volume-expansion.html#policy-precedence["政策優先順序"]。
...
將 PVC 註釋設定為 `"none"`。此設定會覆蓋該磁碟區的任何 StorageClass 層級原則：
...
Trident 停止擴充磁碟區。即使使用量持續增加超過 `usedThreshold`，它也不會再為該磁碟區建立任何調整大小的請求。

例如，根據此策略，Trident 在容量達到 100 GiB 後將停止成長：
...
是的。更新原則後，所有使用此原則的磁碟區都會在下次成長評估時採用新參數。無需重新啟動磁碟區。
...
策略使用終結器保護。如果磁碟區仍然使用該策略，刪除操作將進入 `Deleting`狀態，並等待您將所有磁碟區從該策略中移除。
...
kubectl annotate pvc <pvc-name> \
  trident.netapp.io/autogrowPolicy-
----
移除所有磁碟區後，Trident 會釋放終結器並刪除政策。



=== 自動增長功能支援哪些後端？

Autogrow 支援 ONTAP 後端。也支援 Google Cloud NetApp Volumes （GCNV） 的 NAS 和 SAN 磁碟區。

自動成長功能支援 NFS、iSCSI、FCP 和 NVMe 協定。NVMe 原始區塊磁碟區需要 ONTAP 9.16.1 或更新版本。

對於現有部署環境中的磁碟區，可能需要先完成磁碟區發布移轉，然後自動增長功能才能生效。若要驗證移轉狀態，請檢查 Trident 控制器日誌：
