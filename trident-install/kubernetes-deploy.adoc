...
[]
=====
* Trident 現在支援 Kubernetes。請先升級 Trident，再升級 Kubernetes。
...
無論手動部署或使用 Helm，Trident operator 都是簡化安裝和動態管理 Trident 資源的絕佳方式。您甚至可以link:kubernetes-customize-deploy.html["自訂您的Trident營運者部署"]使用 `TridentOrchestrator`自訂資源（CR）中的屬性。
...
* 如果你有一個 `tridentctl`基於部署，無需卸載Trident即可進行基於 Helm 或基於 Operator 的部署。
