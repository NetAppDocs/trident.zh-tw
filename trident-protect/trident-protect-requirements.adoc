...
* 確保至少存在一個 VolumeSnapshotClass。如需詳細資訊、請參閱 link:https://docs.netapp.com/us-en/trident/trident-reference/objects.html#kubernetes-volumesnapshotclass-objects["Volume SnapshotClass"]。
...
----
====
若要了解如何使用 Trident Protect 定義應用程式和還原 VM，請參閱link:trident-protect-kubevirt-vms.html["使用 Trident Protect 保護 KubeVirt 虛擬機器"]。
