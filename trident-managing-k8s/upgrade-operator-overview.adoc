...
安裝和升級 Trident 的眾多link:../trident-install/kubernetes-deploy.html["使用 Trident 運算子的優點"]之一是能夠自動處理 Trident 和 Kubernetes 物件，而不會中斷現有掛載的磁碟區。這樣，Trident 可以支援零停機升級，或link:https://kubernetes.io/docs/tutorials/kubernetes-basics/update/update-intro/["滾動更新 _"^]。具體來說，Trident Operator 與 Kubernetes 叢集通訊以：
