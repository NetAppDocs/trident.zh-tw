...
* 若要從歸檔中復原特定的 KubeVirt 虛擬機器 (VM) 而不還原所有命名空間資源，請使用復原 CR 上的 `includedVirtualMachines`欄位。有關支援的 CR、欄位參考和 YAML 範例，請參閱link:trident-protect-kubevirt-vms.html["使用 Trident Protect 保護 KubeVirt 虛擬機器"]。
...
** *spec.destinationApplicationName*：（_選用_）還原應用程式的名稱。如果提供、還原的應用程式會使用此名稱。如果未提供、還原的應用程式會使用來源應用程式名稱。
...
NOTE: 如果目標叢集上不存在 AppVault CR，請按照 link:trident-protect-appvault-custom-resources.html["使用Trident Protect AppVault 物件來管理儲存桶。"] 中的步驟建立它。
...
spec:
  ...
  appArchivePath: my-snapshot-path
----
+ .（可選）如果您只需要選擇應用程式的某些資源進行恢復，請新增篩選條件，以包含或排除帶有特定標籤的資源：

+ 注意： Trident Protect 會自動選擇某些資源，因為它們與您選擇的資源有關聯。例如，如果您選擇了持續性磁碟區宣告資源，且該資源關聯了一個 Pod，則 Trident Protect 也會還原該關聯 Pod。

+ * *resourceFilter.resourceSelectionCriteria*: （過濾必要）使用 `Include`或 `Exclude`來包含或排除在 resourceMatchers 中定義的資源。新增下列 resourceMatchers 參數來定義要包含或排除的資源：** *resourceFilter.resourceMatchers*：resourceMatcher 物件的陣列。如果在此陣列中定義多個元素，它們之間將按 OR 運算匹配，每個元素內的欄位（group、kind、version）之間將按 AND 運算匹配。*** *resourceMatchers[].group*: （_可選_）要篩選的資源群組。*** *resourceMatchers[].kind*: （_可選_）要篩選的資源種類。*** *resourceMatchers[].version*: （_可選_）要篩選的資源版本。*** *resourceMatchers[].names*: （_可選_）要篩選的資源的 Kubernetes metadata.name 欄位中的名稱。*** *resourceMatchers[].namespaces*: （_可選_）要篩選的資源的 Kubernetes metadata.name 欄位中的命名空間。*** *resourceMatchers[].labelSelectors*: （_可選_）資源的 Kubernetes metadata.name 欄位中的標籤選擇器字串，如 https://kubernetes.io/docs/concepts/overview/working-with-objects/labels/#label-selectors["Kubernetes文件"^]中所定義。例如： `"trident.netapp.io/os=linux"`。
